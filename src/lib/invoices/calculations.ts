...
/**
 * The Slice 1 dual-write compatibility contract for today's flat (non-
 * itemized) Invoice create/edit Server Actions
 * (docs/invoicing-architecture.md §12 step 3): every newly created or
 * edited flat invoice writes `subtotal = amount`, a zeroed discount/tax,
 * and `taxLabel = TAX` — Decimal end to end, never JS floating point.
 * `amount` itself is passed straight through as the same trusted,
 * already-validated string/Decimal the caller already uses to write the
 * existing `amount` column — this performs no additional validation
