...
async function processMessages(cronToken: string) {
  const { data: expected, error: tokenError } = await admin.rpc('get_whatsapp_job_token');
  if (tokenError || !expected || cronToken !== expected) return json({ error: 'Não autorizado' }, 401);
  ...
  const { data: pending, error: pendingError } = await admin.from('whatsapp_message_logs')
  ...
  const { data: sentRows } = await admin.from('whatsapp_message_logs')
    .select('organization_id').eq('status', 'sent')
    .gte('sent_at', `${today}T00:00:00.000Z`).lt('sent_at', `${today}T23:59:59.999Z`).limit(5000);
  for (const row of sentRows || []) sentToday.set(row.organization_id, (sentToday.get(row.organization_id) || 0) + 1);

  let sent = 0, failed = 0, skipped = 0;
  for (const row of pending || []) {
    const { data: configs } = await admin.rpc('get_whatsapp_delivery_config', { p_organization_id: row.organization_id });
    const config = Array.isArray(configs) ? configs[0] : configs;
    if (!config?.enabled || !config?.phone_number_id || !config?.access_token) { skipped++; continue; }
    const current = sentToday.get(row.organization_id) || 0;
    const limit = Number(config.daily_limit || 0);
    if (limit > 0 && current >= limit) { skipped++; continue; }

    try {
      await sendText(config.phone_number_id, config.access_token, row.phone, row.message);
      await admin.from('whatsapp_message_logs').update({ status: 'sent', sent_at: new Date().toISOString(), updated_at: new Date().toISOString(), error_message: null }).eq('id', row.id);
      sentToday.set(row.organization_id, current + 1);
      sent++;
    } catch (error) {
      await admin.from('whatsapp_message_logs').update({ status: 'failed', error_message: error instanceof Error ? error.message : 'Falha desconhecida', updated_at: new Date().toISOString() }).eq('id', row.id);
      failed++;
    }
  }
...
Deno.serve(async (req) => {
  ...
  if (body?.action === 'process') return processMessages(req.headers.get('x-gestorpro-cron') || '');

  if (body?.action === 'test') {
    const user = await authenticateUser(req);
    if (!user) return json({ error: 'Sessão inválida' }, 401);
    const { data: profile, error: profileError } = await admin.from('profiles').select('organization_id').eq('id', user.id).maybeSingle();
    ...
    const { data: configs } = await admin.rpc('get_whatsapp_delivery_config', { p_organization_id: profile.organization_id });
    const config = Array.isArray(configs) ? configs[0] : configs;
    if (!config?.phone_number_id || !config?.access_token) return json({ error: 'Configure o Phone Number ID e o Access Token antes do teste.' }, 400);
