...
- Staff can create/edit/delete flat and itemized `DRAFT` invoices, manage the seven allowed transitions among existing non-`DRAFT` legacy invoices, and duplicate a `CANCELLED` invoice into a fresh `DRAFT` — all implemented end to end (Slice 1, Slice 2a, Slice 2b, and Duplicate-as-new-DRAFT; PR #65/#67/#68/#70); **official Invoice System Slice 2 is complete** — but there is still no combined Issue/finalization-and-PDF-archival operation (official Slice 3), and no send-by-email flow (official Slice 4). That's official Slices 3–5, not started.
- Billing reconciliation and trial-ending reminder notifications don't exist yet.
