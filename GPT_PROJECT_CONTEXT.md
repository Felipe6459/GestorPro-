...
This document was re-swept after PR #68 to keep every affected claim accurate — model/enum/migration/line counts, current test totals, and the Invoice feature status (Slices 1/2a/2b complete, Slices 3–5 unstarted, the Duplicate-as-new-DRAFT remainder of the originally-scoped Slice 2 deliberately deferred — see Part 11) are all updated below to reflect this baseline, not left as stale open items. A later, purely documentation-only commit that updates this file under version control does not by itself change the application-state baseline stated above — it remains `b3da990400960603136dc6a6038a58041cbc3933` until a future application change lands.
...
**This remains a multi-slice feature in progress, no longer only a foundation.** The prerequisite repair, the approved architecture, and three of five planned slices (1, 2a, 2b) have shipped — staff can now create, edit, and manage the lifecycle of both flat and itemized `DRAFT`/non-`DRAFT` invoices end to end. What a reader might still expect from "Invoice System" work and does **not** yet exist: the actual Issue/finalization operation, immutable PDF archival, invoice email send/resend, and Duplicate-as-new-DRAFT (the one remaining piece of the originally-scoped "Slice 2," deliberately deferred out of both 2a and 2b — informally tracked in this project's own development history as "Slice 2c," though `docs/invoicing-architecture.md` itself has not been amended to name that sub-slice split). See Part 11 for the precise current-vs-target distinction and Part 28 for exactly what's left.
...
Route: `(dashboard)/invoices/`. Organization-scoped, `src/lib/validation/invoice.ts`. **As of Invoice System Slice 2b (PR #68), the staff-facing Invoice workflow is a real DRAFT-lifecycle itemized-or-flat system, not flat-amount CRUD.** Read this section together with Part 2's Phase 23 and `docs/invoicing-architecture.md` for the full target design and what's still ahead (Duplicate-as-new-DRAFT, Issue/finalization, PDF, email — see Part 28).
...
- **Invoice System — Slice 1 (schema/calculation/currency/Client-billing/flat-dual-write foundation, PR #65), Slice 2a (client-bundle-safe calculation/currency code plus a pure, unwired lifecycle-transition/line-item-form foundation, PR #67), and Slice 2b (real staff-facing flat+itemized `DRAFT` create/edit, server-side total recomputation, non-`DRAFT` lifecycle enforcement with an "Issued" view and Cancel, PR #68) are all complete.** What remains: **Duplicate-as-new-DRAFT** — the one piece of the originally-scoped "Slice 2" deferred out of both 2a and 2b, informally tracked in this project's own development history as "Slice 2c" though `docs/invoicing-architecture.md` itself has not been amended to name that split (§3.2 still describes "Cancel + Duplicate-as-new-DRAFT" as one undivided mechanism) — followed by the real Issue/finalization operation and `issuerSnapshot`/`recipientSnapshot` writes (Slice 3), PDF generation/archival/download (Slice 3), Invoice send/resend email and an `InvoiceEmailAttempt` writer (Slice 4), and the final `NOT NULL` contract on the calculated-total columns, the organization-wide invoice-numbering constraint, the corrected portal DRAFT-visibility fix, and final whole-feature test/seed/doc closure (Slice 5) — all still unstarted. The two Invoice migrations merged so far (`20260911090000_repair_invoice_organization_scope`, `20260912090000_add_invoice_system_slice1_foundation` — neither Slice 2a nor Slice 2b added a new migration) still require a separate, controlled deployment/operator step against any real external/staging/production database — neither has been applied outside this repository's local ephemeral test harness. See `docs/invoicing-architecture.md` for the complete slice plan.
...
- Staff can now genuinely create/edit/manage flat and itemized `DRAFT` invoices and manage the lifecycle of existing non-`DRAFT` invoices end to end (Slices 1/2a/2b, PR #65/#67/#68) — but there is still no Duplicate-as-new-DRAFT, no actual Issue/finalization operation, no PDF export, and no send-by-email flow. That's the remaining Slice 2 item plus Invoice System Slices 3–5, not started.
- Billing reconciliation and trial-ending reminder notifications don't exist yet.
...
**If invoicing work continues, the immediate likely next task is Duplicate-as-new-DRAFT** (the one remaining piece of the originally-scoped "Slice 2," deferred out of both 2a and 2b), followed by the real Issue/finalization operation (Slice 3) — explicitly **not** PDF/email, which come later in Slice 3/Slice 4 — see `docs/invoicing-architecture.md` and Part 28. **Paddle/billing is a separate, already-implemented, deliberately deferred track** (live sandbox validation is its own gap, unrelated to and not blocking Invoice System work) — do not begin or redesign Paddle work as part of continuing the Invoice System.
