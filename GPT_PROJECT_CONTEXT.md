...
**Updated against:** `main` @ commit `91234ade4cd5f9bc6cf28531e4355640a492ea56` (149 commits at content-update time; all pre-existing tracked files clean). One application/test-quality PR landed since the original investigation: **PR #58** (`13c480dca64428c66fcb9d05e340ae4d175d133b`, merged as `91234ade4cd5f9bc6cf28531e4355640a492ea56`) — a small housekeeping/test-quality change that resolved the disposition of two previously-stale branches. It changed exactly one line, in one E2E test file, and no application code, schema, or product behavior — see Part 2's Phase 21 and Part 24 for what it did and did not change. This document was re-swept after PR #58 to keep every affected claim (not just the branch-status ones) accurate; nothing below currently reflects the pre-PR-#58 "two unmerged branches" state. PR #58 remains the last application/test-quality change reflected in this content, until a future content update says otherwise.
...
**Maturity assessment (evidence-based, detailed in Part 26):** this is a **functional, feature-complete MVP-to-early-SaaS-grade product**, not a prototype and not yet a fully hardened production SaaS with paying customers. It has real multi-tenant data isolation, a real (but not live-validated) payment integration, 1,855 automated tests, 14 dedicated security checks, and a clean lint/typecheck/build — but it has never processed a real transaction, never had a real customer, and carries a short, explicitly disclosed list of known gaps (custom-domain verification, invoice PDF export, cross-instance rate limiting, billing reconciliation).

---

## PART 2 — Development History (reconstructed from git log; 147 commits as of the original investigation, 2026-07-28 → 2026-08-14; 149 commits as of this update)
...
**Database:** PostgreSQL via Supabase, accessed exclusively through Prisma (`@prisma/client` 7.9.1, `@prisma/adapter-pg`). 25 models, 20 versioned SQL migrations under `prisma/migrations/`. `DATABASE_URL` (pgbouncer pooled) is what the running app uses; `DIRECT_URL` (unpooled) is what Prisma CLI migrations use — configured in `prisma.config.ts`.
...
Full schema: `prisma/schema.prisma` (1,277 lines, 25 models, 20 enums, 20 migrations). Every business model below carries `organizationId` for tenant scoping unless noted.
...
| `PortalUser` | **A separate client-facing login identity** (id = its own Supabase auth user id) | belongs to exactly one `Client`, never has a `Membership`, never resolved by staff auth helpers |
...
- A comprehensive automated test suite (1,855 tests) including dedicated authorization/tenant-isolation integration tests (`test/integration/authorization/`, `test/integration/security/`) and E2E security-UI specs (redirect-safety, XSS-as-literal-text rendering).
...
- **Database migrations:** 20 versioned SQL migrations under `prisma/migrations/`, applied via standard Prisma CLI workflow (`DIRECT_URL`, unpooled connection).
...
| Analytics (portal) | ✅ | ✅ | mostly ✅ | ✅ | mostly ✅ | **MOSTLY COMPLETE** (its own commit message flagged 2 metrics as needing new persistence at the time; not independently re-verified metric-by-metric in this pass — see Part 24) |
...
- **Analytics Portal metrics:** the commit that introduced Portal analytics explicitly flagged 2 metrics as "requiring new persistence" at the time it shipped (Phase 13). This investigation did not re-verify metric-by-metric whether that gap was later closed by a subsequent commit — flagged as **unknown, needs verification** rather than asserted either way.
...
**The engineering process itself is mature.** 1,855 automated tests (unit, integration against a real Postgres, and full end-to-end browser tests) currently pass, alongside a clean lint run, a clean TypeScript check, and a successful production build — all independently re-verified during this investigation, not taken on faith from prior notes. Two GitHub Actions workflows gate every change.
...
**What already works, end to end, right now:** every core CRM workflow, real multi-tenant isolation, the full Client Portal, a real (if never live-fired) billing integration, notifications, comments, search, analytics, onboarding, and a clean automated-quality bar (lint/typecheck/build/1,855 tests all currently green).
...
4. One flagged-but-unverified Analytics gap (Portal metrics needing "new persistence," per Phase 13's own commit message) remains an open loose end — not re-verified as closed or still-open by any investigation to date.
...
- The 14 `scripts/security-checks/` static checks and the 1,855-test suite — treat a red result here as a real regression signal, and extend this pattern (a new invariant gets a new check) rather than inventing a different verification mechanism.
...
- Re-verify the "2 metrics requiring new persistence" note from the Analytics-Portal commit (Phase 13) — confirm current status rather than assume it either way.
...
You are continuing development of an **existing, substantially-built** multi-tenant CRM SaaS application called **Client Portal CRM**. It is not a new project and should not be treated as a blank slate. The following has already been built and verified working (lint clean, TypeScript clean, production build succeeds, 1,855 automated tests passing, 14 dedicated security checks passing — all as of the original full investigation at commit `d76cd51c54594ae0f5743b1918fbae60ac8c1126` on `main`). `main` has since moved one small step further, to `91234ade4cd5f9bc6cf28531e4355640a492ea56` (PR #58) — a housekeeping change, described below, that touched one test file and nothing else; it did not change any of the figures or facts in this paragraph.
...
**Database:** 25 Prisma models, 20 migrations. Every business entity carries `organizationId` for tenant scoping. Key models: `User`, `Organization`, `Membership` (role: Owner/Admin/Member), `Invitation`, `Client`, `Project`, `Task`, `Invoice`, `Activity` (append-only audit log), `Notification`/`NotificationDelivery`/`NotificationPreference`, `Attachment`, `Comment`/`CommentMention`, `PortalUser`/`ClientInvitation` (the separate client-facing identity), `Subscription`/`WebhookEvent` (billing), `OrganizationProfile`/`OrganizationPaymentDetails`/`OrganizationDomainSettings`, `OrganizationOnboardingStep`.
...
- The Analytics-Portal feature's own commit history flagged 2 metrics as needing further persistence work at the time it shipped; current status wasn't re-verified metric-by-metric in the most recent investigation. **This one is still genuinely open — do not treat it as resolved.**
