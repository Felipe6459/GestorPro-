...
- `issueDate` is persisted and set by the model's own `@default(now())` write behavior, but is not a current form control — no page or action reads or writes it.
- `paidAt` is server-derived from `status` transitions only (the 4-case rule in both Server Actions) and is never submitted as, or read from, a form field.
